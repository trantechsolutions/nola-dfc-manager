...
const LEAGUE_KEYWORDS = ['psl', 'lsap', 'lcsl', 'st charles league'];
...
/**
 * Extracts the tournament name from an iCal DESCRIPTION field.
 * The tournament name is the last non-empty line, optionally prefixed with
 * "HOLD for " and suffixed with " - Game N", " - Semi-Final", etc.
 * Returns null if nothing meaningful can be extracted.
 */
export function extractTournamentName(description = '') {
  ...
  // Remove "HOLD for " prefix
  name = name.replace(/^HOLD\s+for\s+/i, '').trim();
  // Remove game-number / round suffix  e.g. "- Game 2", "- Semi-Final Game", "- Final"
  name = name.replace(/\s*[-–]\s*(Game\s*\d+|(Semi[-\s]?|Quarter[-\s]?)?Final(\s+Game)?|3rd\s+Place)\s*$/i, '').trim();
  return name || null;
}
...
export const EVENT_TYPES = {
  ...
  practice: {
    label: 'Practice',
    color: 'bg-slate-400',
    colorLight: 'bg-slate-100 text-slate-600',
