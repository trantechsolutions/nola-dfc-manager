...
// Season runs August 1 of the first year to May 31 of the second year.
...
export function getSeasonDateRange(seasonId) {
  ...
  const end = new Date(endYear, 4, 31, 23, 59, 59); // May 31, 23:59:59
...
export function filterEventsBySeason(events, seasonId) {
  ...
  const filterList = (list) => list.filter(e => 
    e.timestamp >= range.startTimestamp && e.timestamp <= range.endTimestamp
  );
