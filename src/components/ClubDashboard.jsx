...
export default function ClubDashboard({ club, teams, seasons, selectedSeason, onSelectTeam }) {
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-6">
      ...
      {/* Summary Cards — operational info only, no financials */}
      ...
      {/* Per-Team Cards — no financial data */}
        ...
        {teams.map(team => {
          ...
          return (
            <div key={team.id}
                  ...
                  {d.playerCount > 0 && (
                    <div className="mt-2 flex items-center gap-2">
                      <div className="flex-grow h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${d.complianceRate === 100 ? 'bg-emerald-500' : 'bg-amber-400'}`}
                          style={{ width: `${d.complianceRate}%` }} />
                      </div>
                      <span className={`text-[10px] font-black ${d.complianceRate === 100 ? 'text-emerald-600' : 'text-amber-600'}`}>
                        {d.complianceRate}%
                      </span>
                    </div>
                  )}
              ...
              {/* Missing compliance callout */}
              {hasMissing && (
                <div className="mt-3 pt-3 border-t border-slate-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    <AlertTriangle size={12} className="text-amber-500" />
                    <span className="text-[10px] font-black text-amber-600 uppercase">Missing Compliance ({d.missingCompliance.length})</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {d.missingCompliance.slice(0, 6).map((p, i) => (
                      <span key={i} className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 rounded px-2 py-0.5">
                        <span className="text-[10px] font-bold text-slate-600">#{p.jersey || '?'} {p.name}</span>
                    ...
                    {d.missingCompliance.length > 6 && (
                      <span className="text-[10px] font-bold text-slate-400">+{d.missingCompliance.length - 6} more</span>
                    )}
                  </div>
                </div>
              )}
