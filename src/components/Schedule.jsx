...
export default function Schedule({ events }) {
  ...
  const renderCard = (event, isPast = false) => {
    ...
    return (
      <div key={event.id} className={`bg-white rounded-xl border overflow-hidden transition-all ${
        isPast ? 'opacity-60 border-slate-200' : `${type.border} hover:shadow-md hover:-translate-y-0.5`
        ...
        <div className="p-4">
          ...
          <h4 className={`font-black text-slate-900 text-sm leading-tight mb-3 ${event.isCancelled ? 'line-through' : ''}`}>
            {event.title}
          </h4>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-start gap-2 text-xs text-slate-500">
              ...
              <span className="font-medium leading-tight" title={event.location}>{event.location}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-5 pb-20 md:pb-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Search events, locations, or details..."
            value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-9 pr-8 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          {searchTerm && (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex flex-wrap gap-1 bg-slate-100 rounded-lg p-0.5">
            <button onClick={() => setTypeFilter('all')}
              className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-all ${typeFilter === 'all' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500'}`}>
              All
            </button>
          ...
          {hasFilters && (
            <button onClick={() => { setTypeFilter('all'); setSearchTerm(''); }}
      ...
      <section>
        <div className="flex justify-between items-end mb-4">
          <div>
            <h2 className="text-xl font-black text-slate-900">Upcoming</h2>
        ...
        {filteredUpcoming.length === 0 ? (
          <div className="bg-white rounded-xl border-2 border-dashed border-slate-200 p-10 text-center text-slate-400 font-bold italic text-sm">
      ...
      {filteredPast.length > 0 && (
        <section>
          <button onClick={() => setShowArchive(!showArchive)}
            className="w-full flex justify-between items-center bg-white hover:bg-slate-50 p-4 rounded-xl transition-all border border-slate-200">
