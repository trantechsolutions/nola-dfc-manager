...
import {
  ...
  Download, RefreshCw, ReceiptText, Trash2, AlertTriangle
} from 'lucide-react';
...
export default function BulkUploadLedgerModal({
  ...
  players, categoryLabels, categoryColors,
  selectedSeason, teamSeasonId,
  onBulkSave, showToast,
}) {
  ...
  const handleApplyMapping = () => {
    if (columnMap.date === undefined || columnMap.title === undefined ||
        columnMap.amount === undefined || columnMap.category === undefined) {
  ...
  const toggleTxn = (rowIndex) => {
    setParsedTxns(prev => prev.map(t =>
      t._rowIndex === rowIndex ? { ...t, _include: !t._include } : t
    ));
  };
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                ...
                <p className="text-xs text-slate-500">
                  Your CSV should include columns for <span className="font-bold">Date, Title, Amount,</span> and <span className="font-bold">Category</span>.
                  Optionally include Player Name, Payment Method, Cleared status, and Notes.
                ...
                {allCategoryCodes.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    ...
                    {allCategoryCodes.map(code => (
                      <span key={code} className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${(categoryColors || {})[code] || 'bg-slate-100 text-slate-600'}`}>
                        {code}
                      </span>
                    ))}
                  </div>
                ...
                {COLUMN_KEYS.map(col => (
                  <div key={col.key} className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="flex-grow text-xs font-bold border border-slate-200 rounded-lg p-2 outline-none focus:ring-2 focus:ring-emerald-500"
                    >
            ...
            <div className="space-y-3">
              {errorCount > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-red-800">{errorCount} row{errorCount > 1 ? 's' : ''} with errors</p>
              ...
              {warningCount > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-amber-800">{warningCount} player name{warningCount > 1 ? 's' : ''} not matched</p>
              ...
              <div className="text-xs font-bold text-slate-500 flex items-center justify-between">
                <span>{includedCount} of {parsedTxns.length} transactions will be imported</span>
                ...
                <div className="max-h-[35vh] overflow-y-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {parsedTxns.map((t) => (
                        <tr key={t._rowIndex} className={`${!t._include || t._hasError ? 'opacity-40' : ''} hover:bg-slate-50/50`}>
                          <td className="p-2">
                            <input type="checkbox" checked={t._include && !t._hasError}
                              disabled={t._hasError}
                              onChange={() => toggleTxn(t._rowIndex)}
                              className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                          </td>
                          <td className="p-2 text-slate-600 whitespace-nowrap">{t.date || '—'}</td>
                          <td className="p-2 font-bold text-slate-800 max-w-[180px] truncate">{t.title || '—'}</td>
                          <td className={`p-2 text-right font-black whitespace-nowrap ${t.amount < 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                            {t.amount < 0 ? '-' : ''}${Math.abs(t.amount || 0).toFixed(2)}
                          </td>
                          <td className="p-2">
                            {t.category ? (
                              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${(categoryColors || {})[t.category] || 'bg-slate-100 text-slate-600'}`}>
                                {t.category}
                              </span>
                            ) : '—'}
                          </td>
                          ...
                          <td className="p-2">
                            {t._hasError ? (
                              <span className="text-[9px] font-black bg-red-100 text-red-700 px-1.5 py-0.5 rounded uppercase" title={t._errors.join(', ')}>
                                Error
                              </span>
                            ) : (
              ...
              {errorCount > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3">
                  ...
                  <div className="space-y-1 max-h-24 overflow-y-auto">
                    {parsedTxns.filter(t => t._hasError).map(t => (
                      <p key={t._rowIndex} className="text-[11px] text-red-700">
                        <span className="font-bold">Row {t._rowIndex + (hasHeaderRow ? 2 : 1)}:</span> {t._errors.join('; ')}
                      </p>
                    ))}
                  </div>
                </div>
          ...
          {step === 'mapping' && (
            <>
              ...
              <button onClick={handleApplyMapping}
                className="px-5 py-2 text-sm font-black text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 shadow-lg transition-all">
                Preview Import →
              </button>
            </>
          )}
