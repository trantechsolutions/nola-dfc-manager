...
// ── Default system categories (cannot be deleted, but can be edited) ──
export const DEFAULT_CATEGORIES = {
  TMF: { label: 'Team Fees',       description: 'Player team fee payments',                  color: 'bg-blue-50 text-blue-700' },
  SPO: { label: 'Sponsorship',     description: 'Sponsor contributions and donations',       color: 'bg-violet-50 text-violet-700' },
  ...
  TRF: { label: 'Transfer',        description: 'Internal fund transfers between accounts',  color: 'bg-indigo-50 text-indigo-700' },
};
...
export default function CategoryManager({ customCategories, onSave, onDelete, isSaving }) {
  ...
  const validateCode = (code) => {
    if (!code || code.length < 2 || code.length > 5) return 'Code must be 2-5 characters';
    if (!/^[A-Z0-9]+$/.test(code)) return 'Code must be uppercase letters/numbers only';
    // Check against defaults (only on new categories)
    if (!editingId && DEFAULT_CATEGORIES[code]) return 'This code is reserved by a system category';
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={handleNew}
          className="flex items-center gap-1.5 bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-800 transition-all">
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">System Categories (built-in)</p>
        <div className="flex flex-wrap gap-2">
          {Object.entries(DEFAULT_CATEGORIES).map(([code, cat]) => (
            <span key={code} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold ${cat.color}`}>
      ...
      {allCustom.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden divide-y divide-slate-100">
          {allCustom.map((cat) => (
            <div key={cat.id} className="flex items-center gap-3 p-4 hover:bg-slate-50/50 transition-colors">
              <GripVertical size={14} className="text-slate-300 shrink-0" />
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold shrink-0 ${cat.color}`}>
                {cat.code}
              </span>
      ...
      {allCustom.length === 0 && !showForm && (
        <div className="text-center py-8 text-slate-400">
          ...
          <p className="text-xs mt-1">Add categories like "Equipment", "Uniform", or "Travel" to organize your ledger.</p>
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white rounded-xl border-2 border-blue-200 p-5 space-y-4 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  value={formData.code}
                  onChange={e => setFormData({ ...formData, code: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5) })}
                  placeholder="e.g. EQP"
                  maxLength={5}
                  disabled={!!editingId}
                  className="w-full border border-slate-200 rounded-lg p-2 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500 uppercase disabled:bg-slate-50 disabled:text-slate-400"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-lg p-2 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Default Flow</label>
                <select
                  value={formData.flow}
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Color</label>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" disabled={isSaving}
                className="flex items-center gap-1.5 px-5 py-2 bg-blue-600 text-white text-sm font-black rounded-lg hover:bg-blue-700 disabled:opacity-50 shadow-sm">
