...
export const supabaseService = {
  ...
  addPlayer: async (playerData) => {
    const { data: player, error: pErr } = await supabase.from('players').insert({
      first_name: playerData.firstName, last_name: playerData.lastName,
      jersey_number: playerData.jerseyNumber || null, status: playerData.status || 'active',
      medical_release: playerData.medicalRelease || false, reeplayer_waiver: playerData.reePlayerWaiver || false,
    ...
    if (playerData.guardians?.length > 0) {
      const rows = playerData.guardians.filter(g => g.name).map(g => ({
        player_id: player.id, name: g.name, email: g.email || null, phone: g.phone || null
      }));
      if (rows.length > 0) { const { error } = await supabase.from('guardians').insert(rows); if (error) throw error; }
    }

    if (playerData.seasonProfiles) {
      const rows = Object.entries(playerData.seasonProfiles).map(([seasonId, profile]) => ({
        player_id: player.id, season_id: seasonId,
        fee_waived: profile.feeWaived ?? false, status: profile.status || 'active',
        ...(profile.teamSeasonId ? { team_season_id: profile.teamSeasonId } : {}),
      }));
      if (rows.length > 0) { const { error } = await supabase.from('player_seasons').insert(rows); if (error) throw error; }
    }
    return player;
  },
  ...
  updatePlayer: async (playerId, playerData) => {
    ...
    if (playerData.guardians) {
      ...
      const rows = playerData.guardians.filter(g => g.name).map(g => ({
        player_id: playerId, name: g.name, email: g.email || null, phone: g.phone || null
      }));
      if (rows.length > 0) { const { error } = await supabase.from('guardians').insert(rows); if (error) throw error; }
    }
  ...
  updatePlayerField: async (playerId, field, value) => {
    const map = { medicalRelease: 'medical_release', reePlayerWaiver: 'reeplayer_waiver', status: 'status' };
    const { error } = await supabase.from('players').update({ [map[field] || field]: value }).eq('id', playerId);
    if (error) throw error;
  },

  updateSeasonProfile: async (playerId, seasonId, updates) => {
    // player_seasons only has: fee_waived, status, team_season_id (NO base_fee)
    const dbUpdates = {};
    if ('feeWaived' in updates) dbUpdates.fee_waived = updates.feeWaived;
    if ('status' in updates) dbUpdates.status = updates.status;
    if ('teamSeasonId' in updates) dbUpdates.team_season_id = updates.teamSeasonId;
    const { error } = await supabase.from('player_seasons').update(dbUpdates).eq('player_id', playerId).eq('season_id', seasonId);
    if (error) throw error;
  },
  ...
  removePlayerFromSeason: async (playerId, seasonId) => {
    const { error } = await supabase.from('player_seasons').delete().eq('player_id', playerId).eq('season_id', seasonId);
    if (error) throw error;
  },
  ...
  getAllTransactions: async () => {
    const { data, error } = await supabase.from('transactions').select(`*, players (first_name, last_name)`).order('date', { ascending: false });
    if (error) throw error;
    return data.map(tx => ({
      id: tx.id, seasonId: tx.season_id, playerId: tx.player_id,
  ...
  addTransaction: async (txData) => {
    const { data, error } = await supabase.from('transactions').insert({
      season_id: txData.seasonId, player_id: txData.playerId || null,
      date: txData.date, split: txData.split || null, type: txData.type || null,
      category: txData.category, title: txData.title, amount: txData.amount,
      notes: txData.notes || null, cleared: txData.cleared ?? false,
  ...
  deleteBatch: async (field, value) => {
    const dbField = field === 'waterfallBatchId' ? 'waterfall_batch_id' : field === 'originalTxId' ? 'original_tx_id' : field;
    const { error } = await supabase.from('transactions').delete().eq(dbField, value);
    if (error) throw error;
  },

  // ─────────────────────────────────────────
  // SEASONS (only: id, name, created_at, updated_at)
  // ─────────────────────────────────────────
  ...
  saveSeason: async (seasonId, data) => {
    const { error } = await supabase.from('seasons').upsert({ id: seasonId, name: data.name || seasonId }, { onConflict: 'id' });
    if (error) throw error;
  },
  ...
  getBudgetItems: async (seasonId) => {
    const { data, error } = await supabase.from('budget_items').select('*').eq('season_id', seasonId);
    if (error) throw error;
    return data.map(item => ({
      id: item.id, category: item.category, label: item.label,
      income: Number(item.income), expensesFall: Number(item.expenses_fall),
      expensesSpring: Number(item.expenses_spring), teamSeasonId: item.team_season_id,
    }));
  },
  ...
  saveBudgetItems: async (seasonId, items) => {
    ...
    if (items.length > 0) {
      const rows = items.map(item => ({
        season_id: seasonId, category: item.category, label: item.label,
        income: item.income || 0, expenses_fall: item.expensesFall || 0,
  ...
  // ─────────────────────────────────────────
  // PLAYER FINANCIALS (snapshot written on finalization)
  // ─────────────────────────────────────────

  // player_financials is a VIEW — auto-computed from team_seasons + player_seasons + transactions.
  // No writes needed. Just read from it.
  ...
  getPlayerFinancials: async (seasonId, teamSeasonId = null) => {
    ...
    if (error) throw error;
    return data.map(f => ({
      playerId: f.player_id, seasonId: f.season_id, teamSeasonId: f.team_season_id,
      feeWaived: f.fee_waived, baseFee: Number(f.base_fee || 0),
      totalPaid: Number(f.total_paid || 0), fundraising: Number(f.fundraising || 0),
      sponsorships: Number(f.sponsorships || 0), credits: Number(f.credits || 0),
      remainingBalance: Number(f.remaining_balance || 0),
    }));
  },
  ...
  saveBlackout: async (dateStr) => {
    const { error } = await supabase.from('blackouts').upsert({ date_str: dateStr, is_blackout: true }, { onConflict: 'date_str' });
    if (error) throw error;
  },
  ...
  getClubForUser: async () => {
    ...
    if (!user) return null;
    const { data: roles, error } = await supabase.from('user_roles').select('club_id, team_id, teams(club_id)').eq('user_id', user.id).limit(1);
  ...
  getTeams: async (clubId) => {
    ...
    if (error) throw error;
    return data.map(t => ({
      id: t.id, clubId: t.club_id, name: t.name, ageGroup: t.age_group,
      gender: t.gender, tier: t.tier, icalUrl: t.ical_url, colorPrimary: t.color_primary, status: t.status,
    }));
  },
  ...
  getTeam: async (teamId) => {
    ...
    if (error) throw error;
    return {
      id: data.id, clubId: data.club_id, name: data.name, ageGroup: data.age_group,
      gender: data.gender, tier: data.tier, icalUrl: data.ical_url, colorPrimary: data.color_primary, status: data.status,
    };
  },

  createTeam: async (teamData) => {
    const { data, error } = await supabase.from('teams').insert({
      club_id: teamData.clubId, name: teamData.name, age_group: teamData.ageGroup || null,
      gender: teamData.gender || null, tier: teamData.tier || null,
      ical_url: teamData.icalUrl || '', color_primary: teamData.colorPrimary || '#1e293b',
    }).select().single();
    if (error) throw error;
    return data;
  },
  ...
  getTeamSeasons: async (teamId) => {
    const { data, error } = await supabase.from('team_seasons').select('*').eq('team_id', teamId).order('season_id', { ascending: false });
    if (error) throw error;
    return data.map(ts => ({
      id: ts.id, teamId: ts.team_id, seasonId: ts.season_id, isFinalized: ts.is_finalized,
      baseFee: ts.base_fee ? Number(ts.base_fee) : 0, bufferPercent: ts.buffer_percent,
  ...
  getTeamSeason: async (teamId, seasonId) => {
    const { data, error } = await supabase.from('team_seasons').select('*').eq('team_id', teamId).eq('season_id', seasonId).maybeSingle();
    if (error) throw error;
    if (!data) return null;
    return {
      id: data.id, teamId: data.team_id, seasonId: data.season_id, isFinalized: data.is_finalized,
      baseFee: data.base_fee ? Number(data.base_fee) : 0, bufferPercent: data.buffer_percent,
  ...
  saveTeamSeason: async (teamSeasonData) => {
    const row = {
      team_id: teamSeasonData.teamId, season_id: teamSeasonData.seasonId,
      is_finalized: teamSeasonData.isFinalized ?? false, base_fee: teamSeasonData.baseFee ?? 0,
      buffer_percent: teamSeasonData.bufferPercent ?? 5, expected_roster_size: teamSeasonData.expectedRosterSize ?? null,
  ...
  getUserRoles: async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return [];
    const { data, error } = await supabase.from('user_roles').select('*, clubs(name, slug), teams(name, age_group, gender)').eq('user_id', user.id);
    if (error) throw error;
    return data.map(r => ({ id: r.id, userId: r.user_id, clubId: r.club_id, teamId: r.team_id, role: r.role, clubName: r.clubs?.name || null, teamName: r.teams?.name || null }));
  },

  getTeamRoles: async (teamId) => {
    const { data: team, error: tErr } = await supabase.from('teams').select('club_id').eq('id', teamId).single();
    if (tErr) throw tErr;
    const { data, error } = await supabase.from('user_roles').select('*').or(`team_id.eq.${teamId},club_id.eq.${team.club_id}`);
    if (error) throw error;

    const userIds = [...new Set(data.map(r => r.user_id))];
    let profileMap = {};
    if (userIds.length > 0) {
      try { const profiles = await supabaseService.getUserProfiles(userIds); profiles.forEach(p => { profileMap[p.userId] = p; }); }
      catch (e) { console.warn('Could not fetch user profiles:', e.message); }
    }
    return data.map(r => {
      const profile = profileMap[r.user_id] || {};
      return { id: r.id, userId: r.user_id, teamId: r.team_id, clubId: r.club_id, role: r.role, displayName: profile.displayName || null, email: profile.email || null, isClubLevel: !r.team_id && !!r.club_id };
    });
  },
  ...
  getUserIdByEmail: async (email) => {
    const { data, error } = await supabase.from('user_profiles').select('user_id').eq('email', email.toLowerCase().trim()).maybeSingle();
    if (error) throw error;
  ...
  getPlayersByTeam: async (teamId) => {
    const { data, error } = await supabase.from('players').select('*, guardians(*), player_seasons(*)').eq('team_id', teamId).order('last_name');
    if (error) throw error;
    return data.map(p => ({
      id: p.id, firstName: p.first_name, lastName: p.last_name, jerseyNumber: p.jersey_number,
      status: p.status, medicalRelease: p.medical_release, reePlayerWaiver: p.reeplayer_waiver,
      clubId: p.club_id, teamId: p.team_id,
      guardians: (p.guardians || []).map(g => ({ id: g.id, name: g.name, email: g.email, phone: g.phone })),
      seasonProfiles: (p.player_seasons || []).reduce((acc, ps) => {
        acc[ps.season_id] = { feeWaived: ps.fee_waived, status: ps.status, teamSeasonId: ps.team_season_id };
  ...
  getTransactionsByTeamSeason: async (teamSeasonId) => {
    const { data, error } = await supabase.from('transactions').select('*, players(first_name, last_name)').eq('team_season_id', teamSeasonId).order('date', { ascending: false });
    if (error) throw error;
    return data.map(tx => ({
      id: tx.id, seasonId: tx.season_id, teamSeasonId: tx.team_season_id, playerId: tx.player_id,
  ...
  getPlayerDocuments: async (playerId) => {
    const { data, error } = await supabase.from('documents').select('*').eq('player_id', playerId).order('created_at', { ascending: false });
    if (error) throw error;
    return data.map(d => ({
      ...
      fileSize: d.file_size, mimeType: d.mime_type, status: d.status,
      verifiedBy: d.verified_by, verifiedAt: d.verified_at, expiresAt: d.expires_at,
      notes: d.notes, uploadedBy: d.uploaded_by, createdAt: d.created_at,
    }));
  },

  getTeamDocuments: async (teamId) => {
    const { data, error } = await supabase.from('documents').select('*, players(first_name, last_name, jersey_number)').eq('team_id', teamId).order('created_at', { ascending: false });
    if (error) throw error;
    return data.map(d => ({
      id: d.id, playerId: d.player_id,
      playerName: d.players ? `${d.players.first_name} ${d.players.last_name}` : null,
      jerseyNumber: d.players?.jersey_number, docType: d.doc_type, title: d.title,
      fileName: d.file_name, filePath: d.file_path, fileSize: d.file_size, mimeType: d.mime_type,
  ...
  uploadDocument: async (file, playerId, docMeta) => {
    ...
    const { error: uploadErr } = await supabase.storage.from('player-documents').upload(storagePath, file, { contentType: file.type });
    if (uploadErr) throw uploadErr;

    const { data: { user } } = await supabase.auth.getUser();
    const { data, error } = await supabase.from('documents').insert({
      player_id: playerId, club_id: docMeta.clubId || null, team_id: docMeta.teamId || null,
      season_id: docMeta.seasonId || null, doc_type: docMeta.docType,
      title: docMeta.title || file.name, file_name: file.name,
      file_path: storagePath, file_size: file.size, mime_type: file.type,
  ...
  deleteDocument: async (docId, filePath) => {
    if (filePath) await supabase.storage.from('player-documents').remove([filePath]);
  ...
  getDocumentUrl: async (filePath) => {
    const { data, error } = await supabase.storage.from('player-documents').createSignedUrl(filePath, 3600);
    if (error) throw error;
  ...
  getTeamGuardiansWithStatus: async (teamId) => {
    const { data: players, error } = await supabase.from('players')
      .select('id, first_name, last_name, jersey_number, guardians(*)').eq('team_id', teamId).eq('status', 'active').order('last_name');
    if (error) throw error;
    ...
    players.forEach(p => {
      (p.guardians || []).forEach(g => {
        ...
        if (!guardianMap[email]) {
          guardianMap[email] = { guardianId: g.id, name: g.name, email, phone: g.phone, players: [], userId: null, hasAccount: false, roles: [] };
        }
        guardianMap[email].players.push({ id: p.id, name: `${p.first_name} ${p.last_name}`, jersey: p.jersey_number });
      });
    });

    const emails = Object.keys(guardianMap);
    if (emails.length > 0) {
      const { data: profiles } = await supabase.from('user_profiles').select('user_id, email').in('email', emails);
      (profiles || []).forEach(p => {
        const e = p.email.toLowerCase().trim();
        if (guardianMap[e]) { guardianMap[e].userId = p.user_id; guardianMap[e].hasAccount = true; }
      });
