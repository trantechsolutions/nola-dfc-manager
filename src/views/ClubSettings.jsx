  ...
  return (
    <div className="space-y-6 pb-24 md:pb-6 max-w-3xl">
      ...
      <div className="flex items-center gap-3">
        <div className="p-2.5 bg-slate-900 rounded-xl"><Building2 size={20} className="text-white" /></div>
        <div>
          <h2 className="text-2xl font-black text-slate-900">Club Settings</h2>
          <p className="text-xs text-slate-400 font-bold">{club?.name} · {teams.length} teams</p>
        </div>
      </div>
            ...
            {Object.values(rolesByUser).map(({ userId, roles }) => {
              ...
              <div key={userId} className="bg-slate-50 p-3 rounded-xl">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {roles.map(r => (
                    <div key={r.id} className="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg px-2 py-1 group">
                      {r.scope === 'team' && (
                        <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: r.teamColor || '#94a3b8' }} />
                      )}
                      <span className={`text-[9px] font-black uppercase ${
                        r.role === 'club_admin' ? 'text-red-600' :
                        r.role === 'club_manager' ? 'text-violet-600' :
                        r.role === 'team_manager' ? 'text-blue-600' :
                        r.role === 'treasurer' ? 'text-emerald-600' :
                        r.role === 'scheduler' ? 'text-violet-600' :
                        r.role === 'head_coach' ? 'text-amber-600' :
                        'text-slate-500'
                      }`}>{ALL_ROLES[r.role]?.label || r.role}</span>
                      <span className="text-[9px] text-slate-400">{r.scopeName}</span>
                      <button onClick={() => handleRevokeRole(r.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                        <X size={10} />
                      </button>
                    </div>
                  ))}
        ...
        <div className="space-y-2">
          {Object.entries(ALL_ROLES).map(([key, def]) => (
            <div key={key} className="flex items-start gap-3 p-2.5 bg-slate-50 rounded-lg">
              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded shrink-0 mt-0.5 ${
                key === 'club_admin' ? 'bg-red-100 text-red-700' :
                key === 'club_manager' ? 'bg-violet-100 text-violet-700' :
                key === 'team_manager' ? 'bg-blue-100 text-blue-700' :
                key === 'treasurer' ? 'bg-emerald-100 text-emerald-700' :
                key === 'scheduler' ? 'bg-violet-100 text-violet-700' :
                key === 'head_coach' ? 'bg-amber-100 text-amber-700' :
