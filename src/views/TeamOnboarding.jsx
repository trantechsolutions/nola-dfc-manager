...
export default function TeamOnboarding({ club, seasons, onComplete, onCancel, showToast }) {
  ...
  // Step 1: Team info
  const [teamForm, setTeamForm] = useState({ name: '', ageGroup: '', gender: 'boys', tier: 'competitive', colorPrimary: '#1e293b' });
  ...
  const handleTestIcal = async () => {
    if (!icalUrl) { setIcalValid(null); return; }
    try {
      const response = await fetch(icalUrl);
      const text = await response.text();
      setIcalValid(text.includes('BEGIN:VCALENDAR'));
    } catch { setIcalValid(false); }
  };
  ...
  const handleCreateSeason = async () => {
    ...
    try {
      ...
      try { await supabaseService.saveSeason(selectedSeason, { name: selectedSeason }); } catch {}
      // Create team_season
      const ts = await supabaseService.saveTeamSeason({
        teamId: createdTeam.id, seasonId: selectedSeason,
        baseFee: Number(baseFee), isFinalized: false,
      });
    ...
    <div className="max-w-xl mx-auto space-y-6 pb-24 md:pb-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 font-bold">{club?.name} · Step {step + 1} of {STEPS.length}</p>
        </div>
        <button onClick={onCancel} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg"><X size={20} /></button>
      </div>
      ...
      <div className="flex items-center gap-1">
        {STEPS.map((s, i) => (
          <React.Fragment key={s.id}>
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${
            ...
            {i < STEPS.length - 1 && <div className={`h-0.5 flex-grow rounded ${i < step ? 'bg-emerald-300' : 'bg-slate-200'}`} />}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        {step === 0 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input autoFocus type="text" placeholder="e.g. 2014 Boys White" value={teamForm.name} onChange={e => setTeamForm({...teamForm, name: e.target.value})}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input type="text" placeholder="U11" value={teamForm.ageGroup} onChange={e => setTeamForm({...teamForm, ageGroup: e.target.value})}
              ...
              <div>
                ...
                <select value={teamForm.tier} onChange={e => setTeamForm({...teamForm, tier: e.target.value})}
                  className="w-full border border-slate-200 rounded-lg p-2.5 text-sm outline-none mt-1">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1.5">
                {COLORS.map(c => (
                  <button key={c} type="button" onClick={() => setTeamForm({...teamForm, colorPrimary: c})}
            ...
            <button onClick={handleCreateTeam} disabled={isSaving || !teamForm.name.trim()}
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500">Connect an iCal feed from Ollie Sports (or any .ics URL) to sync the team's schedule automatically.</p>
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">iCal URL</label>
              <div className="flex gap-2 mt-1">
                <input type="url" placeholder="https://api.olliesports.com/ical/team-..." value={icalUrl} onChange={e => { setIcalUrl(e.target.value); setIcalValid(null); }}
                  className="flex-grow border border-slate-200 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                <button onClick={handleTestIcal} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-200">Test</button>
              </div>
              {icalValid === true && <p className="text-xs text-emerald-600 font-bold mt-1 flex items-center gap-1"><CheckCircle2 size={12} /> Calendar verified</p>}
              {icalValid === false && <p className="text-xs text-red-500 font-bold mt-1">Invalid or unreachable URL. Check the link and try again.</p>}
            </div>
            <div className="flex gap-2">
              <button onClick={() => setStep(0)} className="flex-1 py-3 text-slate-500 font-bold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-1">
                <ChevronLeft size={16} /> Back
              </button>
              <button onClick={handleSaveIcal} disabled={isSaving}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500">Create the team's first season with a base fee. You can adjust the budget and roster from the Budget view afterward.</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setStep(1)} className="flex-1 py-3 text-slate-500 font-bold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-1">
                <ChevronLeft size={16} /> Back
              </button>
        ...
        {step === 3 && (
          <div className="text-center py-6 space-y-4">
            <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center" style={{ backgroundColor: teamForm.colorPrimary }}>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => onComplete(createdTeam?.id)} 
