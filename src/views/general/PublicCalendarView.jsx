...
import { 
  Calendar, ChevronDown, MapPin, Clock, Share2, Check, 
  Link2, LogIn, Filter, X, List, CalendarDays, Loader2
} from 'lucide-react';
  ...
  try {
    ...
    return vcalendar.getAllSubcomponents('vevent').map(vevent => {
      ...
      return {
        ...
        displayDate: jsDate.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }),
  ...
  const fetchEvents = useCallback(async () => {
    ...
    if (selectedTeamId === 'all') {
      ...
      const results = await Promise.allSettled(
        teams.filter(t => t.icalUrl).map(t => fetchTeamEvents(t))
      );
          ...
          <div className="mt-4 relative">
            <button onClick={() => setShowTeamDropdown(!showTeamDropdown)}
              ...
              <div className="flex items-center gap-2.5">
                {selectedTeam && (
                  <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: selectedTeam.colorPrimary }} />
                )}
                {selectedTeamId === 'all' && (
            ...
            {showTeamDropdown && (
              <div className="absolute left-0 right-0 md:right-auto md:w-80 top-full mt-1 bg-slate-800 rounded-xl border border-slate-700 shadow-2xl z-50 overflow-hidden max-h-72 overflow-y-auto">
                <button onClick={() => handleTeamChange('all')}
                ...
                {teams.map(t => (
                  <button key={t.id} onClick={() => handleTeamChange(t.id)}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400">{t.ageGroup} · {t.gender} · {t.tier}</p>
                    </div>
                  </button>
                ))}
              </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-4">
        <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-1.5 border border-slate-200 max-w-xs">
            ...
            <button onClick={handleCopyLink} className="text-[10px] font-bold text-blue-600 hover:text-blue-800 shrink-0">
              {copied ? 'Copied!' : 'Copy'}
            </button>
          </div>
        </div>
          ...
          <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              {selectedTeamId === 'all' ? (
                ...
                teams.filter(t => t.icalUrl).map(t => (
          ...
          <div className="space-y-2">
            {filteredEvents.length === 0 ? (
              ...
              filteredEvents.map(event => {
                ...
                return (
                  <div key={event.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex hover:shadow-md transition-shadow">
