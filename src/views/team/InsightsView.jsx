...
export default function InsightsView({
  ...
  const analytics = useMemo(() => {
    ...
    seasonTx.forEach((tx) => {
      if (!isCleared(tx)) return;
      const cat = tx.category || 'OTHER';
      categoryActuals[cat] = (categoryActuals[cat] || 0) + Number(tx.amount || 0);
    });

    const totalIncome = seasonTx.filter((tx) => isCleared(tx) && tx.amount > 0).reduce((sum, tx) => sum + tx.amount, 0);
    const totalExpenses = seasonTx
      .filter((tx) => isCleared(tx) && tx.amount < 0)
    ...
    const avgTournamentCost =
      paidTournaments.length > 0 ? paidTournaments.reduce((s, g) => s + g.totalSpent, 0) / paidTournaments.length : 600;
    const paidLeagues = matchReport.groups.filter((g) => g.type === 'league' && g.hasCost);
    const avgLeagueCost =
      paidLeagues.length > 0 ? paidLeagues.reduce((s, g) => s + g.totalSpent, 0) / paidLeagues.length : 200;
  ...
  const buildSystemPrompt = () => {
    ...
    const s = scheduleAnalytics;
    const m = matchReport;
    const catBreakdown = Object.entries(a.categoryActuals)
      .map(([cat, amt]) => `  ${CATEGORY_LABELS[cat] || cat}: ${formatMoney(amt)}`)
      .join('\n');
    const playerSummary = a.playerBalances
      .map(
        (p) =>
          `  #${p.jerseyNumber || '?'} ${p.name}: fee=${formatMoney(p.baseFee)}, paid=${formatMoney(p.totalPaid)}, fundraising=${formatMoney(p.fundraising)}, sponsorships=${formatMoney(p.sponsorships)}, remaining=${formatMoney(p.remainingBalance)}${p.isWaived ? ' [WAIVED]' : ''}`,
      )
      .join('\n');

    const groupSummary = m.groups
      .map((g) => {
...
${playerSummary}`;
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: `${t('insights.connectionError')} ${err.message}` },
      ]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none overflow-hidden">
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-4">
              ...
              <p className="text-xs text-slate-400 mb-4 max-w-xs">
                ...
                payment records.{' '}
                {m.summary.upcomingWithNoCost > 0
                  ? `${m.summary.upcomingWithNoCost} upcoming events still need payment.`
                  : 'All upcoming events are paid.'}
              </p>
            ...
            <>
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role !== 'user' && (
                    <div className="shrink-0 w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-blue-600 flex items-center justify-center mt-0.5">
                  ...
                  {msg.role === 'user' && (
                    <div className="shrink-0 w-7 h-7 rounded-lg bg-slate-300 flex items-center justify-center mt-0.5">
        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-700">
          ...
          >
            <input
              type="text"
              placeholder={apiKey ? t('insights.chatPlaceholder') : t('insights.chatDisabled')}
              value={input}
