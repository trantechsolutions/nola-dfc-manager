...
export default function DocumentManager({
  ...
  if (loading)
    return <div className="p-20 text-center font-bold text-muted-foreground animate-pulse">Loading documents...</div>;
  ...
  return (
    <div className="space-y-5 pb-24 md:pb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div
          className={`p-4 rounded-lg border shadow-sm ${compliantCount === players.length ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-700' : 'bg-amber-50 dark:bg-amber-900/30 border-amber-200 dark:border-amber-700'}`}
        >
          <FileCheck2
            size={16}
            className={
          ...
          <p className="text-xs font-semibold text-muted-foreground">Fully Compliant</p>
        </div>
        <div
          className={`p-4 rounded-lg border shadow-sm ${missingMedical === 0 ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-700' : 'bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-700'}`}
        >
          <FileCheck2
            size={16}
            className={
              missingMedical === 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'
            }
          />
          <p className="text-xl font-bold mt-1">{missingMedical}</p>
        ...
        <div className="p-4 rounded-lg border shadow-sm bg-card border-border">
          ...
          <p className="text-xl font-bold mt-1">{documents.length}</p>
        ...
        <div className="flex gap-1 bg-muted rounded-lg p-0.5">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${filterStatus === f.id ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}
            >
              {f.label}
            </button>
          ))}
      ...
      <div className="space-y-2">
        {filteredPlayers.map((player) => {
          ...
          return (
            <div
              key={player.id}
              className={`bg-card rounded-lg border shadow-sm overflow-hidden ${comp.isComplete ? 'border-emerald-200 dark:border-emerald-700' : 'border-amber-200 dark:border-amber-700'}`}
            >
              <div className="flex items-center gap-3 p-4">
                {/* Jersey */}
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${comp.isComplete ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300'}`}
                >
                  {player.jerseyNumber || '?'}
                </div>
                ...
                {canEdit && (
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${isExpanded ? 'bg-muted text-foreground' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                  >
                    {isExpanded ? (
                      <>
                        <X size={12} /> Close
                    ...
                    <div className="flex gap-2 items-end">
                      <div className="flex-grow">
                        ...
                        <select
                          ...
                          className="w-full border border-border rounded-lg p-2 text-xs font-semibold outline-none mt-1"
                        >
                          {DOC_TYPES.map((t) => (
                      ...
                      <label
                        className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold cursor-pointer transition-all ${isUploading ? 'bg-muted text-muted-foreground' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                      >
                    ...
                    <div className="space-y-1.5">
                      {comp.docs.map((doc) => (
                        ...
                        >
                          ...
                          <span
                            className={`text-xs font-bold uppercase px-1.5 py-0.5 rounded shrink-0 ${STATUS_COLORS[doc.status]}`}
                          >
                            {doc.status}
                          </span>
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              ...
                              className="p-1 text-muted-foreground hover:text-blue-700 dark:text-blue-400"
                              title="View"
                            >
                              <Eye size={14} />
                            </button>
                            {canEdit && doc.status === 'uploaded' && (
                              <button
                                ...
                                className="p-1 text-muted-foreground hover:text-emerald-700 dark:text-emerald-400"
                                title="Verify"
                              >
                            ...
                            {canEdit && doc.status === 'uploaded' && (
                              <button
                                ...
                                className="p-1 text-muted-foreground hover:text-red-700 dark:text-red-400"
                                title="Reject"
                              >
                                <XCircle size={14} />
                              </button>
