...
export default function BudgetView({ selectedSeason, formatMoney, seasons, setSelectedSeason, refreshSeasons, showToast, showConfirm, onDataChange, teamSeasonId, selectedTeamId }) {
  ...
  const handleDeleteSeason = async () => {
    const ok = await showConfirm(`Delete the "${selectedSeason}" draft? This will remove all budget items, roster assignments, and transactions for this season. This cannot be undone.`);
    if (!ok) return;
    setIsSaving(true);
    try {
  ...
  const handleRemoveFromSeason = async (player) => {
    const ok = await showConfirm(`Remove ${player.firstName} ${player.lastName} from the ${selectedSeason} roster? Their player record will remain in the system.`);
    if (!ok) return;
    try {
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl ${isFinalized ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'}`}>
              {isFinalized ? <Lock size={20} /> : <Unlock size={20} />}
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-xl font-black text-slate-900">{selectedSeason}</h2>
                <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${isFinalized ? 'bg-amber-500 text-white' : 'bg-emerald-500 text-white'}`}>
                  {isFinalized ? 'Finalized' : 'Draft'}
                </span>
              </div>
              <p className="text-[10px] text-slate-400 font-bold mt-0.5">
                Aug {selectedSeason.split('-')[0]} – May {selectedSeason.split('-')[1]} · {seasonPlayers.length} players · Fee: {formatMoney(roundedBaseFee)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {!isFinalized && (
              <>
                <button onClick={handleDeleteSeason} disabled={isSaving} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete Draft">
                  <Trash2 size={16} />
                </button>
                ...
                <button onClick={() => handleSaveBudget(true)} disabled={isSaving} className="px-4 py-2 rounded-xl font-black text-xs text-white bg-slate-900 hover:bg-slate-800 shadow-lg flex items-center gap-1.5 disabled:opacity-50">
                  <CheckCircle2 size={14} /> Finalize
                </button>
              </>
            )}
            <button onClick={() => setShowNewSeasonModal(true)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="New Season">
              <Plus size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-1 mt-4 bg-slate-100 p-1 rounded-xl overflow-x-auto no-scrollbar">
          {tabs.map(tab => (
            ...
            >
              <tab.icon size={14} />
              {tab.label}
              {tab.badge != null && <span className="bg-blue-100 text-blue-700 text-[10px] font-black px-1.5 py-0.5 rounded-full">{tab.badge}</span>}
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-slate-900 text-white px-5 py-3">
                <h3 className="font-black text-sm">NOLA DFC 2015 Boys Black</h3>
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">Budget {selectedSeason.replace('-', ' / ')}</p>
              </div>
                        ...
                        <React.Fragment key={cat.code}>
                          <tr className="bg-slate-800 text-white cursor-pointer select-none" onClick={() => toggleCollapse(cat.code)}>
                            <td colSpan={6} className="px-4 py-2">
                              <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest">
                                ...
                                {cat.name}
                                <span className="ml-auto text-slate-400 text-[9px] font-medium normal-case tracking-normal">{items.length} items</span>
                              </div>
                            </td>
                          </tr>
                            ...
                            return (
                              <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                                <td className="px-4 py-1.5">
                                  {isFinalized ? (
                                    <span className="text-xs text-slate-700">{cat.code} - {item.label || <span className="italic text-slate-300">Untitled</span>}</span>
                                  ) : (
                                    <div className="flex items-center gap-2">
                                      <span className="text-[9px] font-bold text-slate-400 w-7">{cat.code}</span>
                                      <input type="text" placeholder="Label..." value={item.label} onChange={(e) => updateItem(item.id, 'label', e.target.value)}
                                        className="flex-grow bg-transparent border-b border-slate-200 focus:border-blue-500 py-1 text-xs outline-none" />
                                    </div>
                                  )}
                                </td>
                                <td className="px-2 py-1.5 text-right">
                                  {!isExp && !isFinalized ? <input type="number" step="0.01" value={item.income || ''} placeholder="0" onChange={(e) => updateItem(item.id, 'income', e.target.value)} className="w-20 text-right bg-emerald-50 border border-emerald-200 rounded px-1.5 py-1 text-xs font-bold outline-none focus:ring-1 focus:ring-emerald-400" />
                                  : <span className="text-xs">{fmt(!isExp ? Number(item.income) || 0 : 0)}</span>}
                                </td>
                                <td className="px-2 py-1.5 text-right">
                                  {isExp && !isFinalized ? <input type="number" step="0.01" value={item.expensesFall || ''} placeholder="0" onChange={(e) => updateItem(item.id, 'expensesFall', e.target.value)} className="w-20 text-right bg-red-50 border border-red-200 rounded px-1.5 py-1 text-xs font-bold outline-none focus:ring-1 focus:ring-red-400" />
                                  : <span className="text-xs">{fmt(isExp ? -(Number(item.expensesFall) || 0) : 0)}</span>}
                                </td>
                                <td className="px-2 py-1.5 text-right">
                                  {isExp && !isFinalized ? <input type="number" step="0.01" value={item.expensesSpring || ''} placeholder="0" onChange={(e) => updateItem(item.id, 'expensesSpring', e.target.value)} className="w-20 text-right bg-red-50 border border-red-200 rounded px-1.5 py-1 text-xs font-bold outline-none focus:ring-1 focus:ring-red-400" />
                                  : <span className="text-xs">{fmt(isExp ? -(Number(item.expensesSpring) || 0) : 0)}</span>}
                                </td>
                                <td className="px-2 py-1.5 text-right text-xs">{fmtNet(itemNet)}</td>
                                <td className="px-2 py-1.5 text-center w-12">
                                  {!isFinalized && <button onClick={() => removeItem(item.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={12} /></button>}
                                </td>
                              </tr>
                            );
                          })}

                          {!collapsed && !isFinalized && (
                            <tr className="border-b border-slate-100">
                              <td colSpan={6} className="px-4 py-1">
                                <button onClick={() => addItem(cat.code)} className="text-[10px] font-bold text-blue-500 hover:text-blue-700 flex items-center gap-1"><Plus size={10} /> Add Item</button>
                              </td>
                            </tr>
                          )}
                          ...
                          {!collapsed && (
                            <tr className="bg-slate-100 font-bold border-b-2 border-slate-200">
                              <td className="px-4 py-2 text-[10px] text-slate-600 italic uppercase">&gt; {cat.name}</td>
                              <td className="px-2 py-2 text-right text-xs">{fmtNet(sub.income)}</td>
                              <td className="px-2 py-2 text-right text-xs">{sub.expensesFall > 0 ? <span className="text-red-500">({formatMoney(sub.expensesFall)})</span> : fmt(0)}</td>
                              <td className="px-2 py-2 text-right text-xs">{sub.expensesSpring > 0 ? <span className="text-red-500">({formatMoney(sub.expensesSpring)})</span> : fmt(0)}</td>
                              <td className="px-2 py-2 text-right text-xs">{fmtNet(sub.net)}</td>
                              <td className="px-2 py-2 text-right text-xs font-black">{actual !== 0 ? <span className={actual < 0 ? 'text-red-500' : 'text-emerald-600'}>{actual < 0 ? `(${formatMoney(Math.abs(actual))})` : formatMoney(actual)}</span> : fmt(0)}</td>
                            </tr>
                          )}
                        </React.Fragment>
                      );
          ...
          <div className="space-y-4 sticky top-4 self-start">
            <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl relative overflow-hidden">
              ...
              <div className="space-y-5 relative z-10">
                <div>
                  <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Roster (Non-Waived)</label>
                  <p className="text-2xl font-black text-blue-400">{rosterSize}</p>
                </div>
                <div>
                  <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Projected Expenses</label>
            ...
            {projections && (
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm mt-4">
                    ...
                    return (
                      <div key={cat.code} className="space-y-1">
                        <div className="flex justify-between items-center">
                          ...
                          {hasData && (
                            <span className={`text-[9px] font-black ${diff > 0 ? 'text-red-500' : diff < 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                              {diff > 0 ? '+' : ''}{formatMoney(diff)}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5">
                          <div className="flex-grow h-1.5 bg-slate-100 rounded-full overflow-hidden relative">
                            {/* Last season actual (background reference) */}
                            <div className="absolute inset-y-0 left-0 bg-violet-200 rounded-full" style={{ width: `${(proj.avgActual / maxVal) * 100}%` }} />
                            {/* Current budget (foreground) */}
                            <div className={`absolute inset-y-0 left-0 rounded-full ${hasData ? (diff > 0 ? 'bg-red-400' : 'bg-emerald-400') : 'bg-slate-300'}`} style={{ width: `${(currentBudgeted / maxVal) * 100}%` }} />
                          </div>
                        </div>
                ...
                <button onClick={() => setActiveTab('projections')} className="mt-3 w-full py-2 text-[10px] font-bold text-violet-600 hover:bg-violet-50 rounded-lg transition-colors">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm text-center">
              <p className="text-2xl font-black text-slate-800">{seasonPlayers.length}</p>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm text-center">
              <p className="text-2xl font-black text-amber-600">{seasonPlayers.filter(p => p.seasonProfiles[selectedSeason]?.feeWaived).length}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
            ...
            <div className="lg:col-span-3 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                {!isFinalized && (
                  <button onClick={() => setShowPlayerForm(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-[10px] font-black rounded-lg hover:bg-blue-700 transition-all shadow-sm">
              ...
              {seasonPlayers.length === 0 ? (
                <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-xl">
                  ...
                  <p className="text-sm font-bold text-slate-400 mb-1">No players yet</p>
                  <p className="text-xs text-slate-400 mb-3">Add existing players from the right panel or create a new one.</p>
                  {!isFinalized && (
                    <button onClick={() => setShowPlayerForm(true)} className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded-lg hover:bg-blue-700">
                      <Plus size={12} className="inline mr-1" /> Add First Player
                    </button>
                  )}
                </div>
              ) : (
                <div className="space-y-1.5">
                  {seasonPlayers
                    ...
                    return (
                      <div key={p.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${isWaived ? 'border-amber-200 bg-amber-50' : 'border-slate-100 bg-slate-50 hover:bg-slate-100'}`}>
                        {/* Jersey */}
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black ${isWaived ? 'bg-amber-200 text-amber-700' : 'bg-slate-200 text-slate-600'}`}>
                          {p.jerseyNumber || '—'}
                        </div>

                        {/* Name */}
                        <div className="flex-grow min-w-0">
                          <p className={`text-xs font-bold ${isWaived ? 'text-amber-700 line-through opacity-70' : 'text-slate-800'}`}>
                            {p.firstName} {p.lastName}
                          </p>
                          {p.guardians?.[0]?.email && (
                            <p className="text-[10px] text-slate-400 truncate">{p.guardians[0].email}</p>
                          )}
                        </div>

                        {/* Actions */}
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button onClick={() => handleToggleWaiver(p.id, isWaived)} disabled={isFinalized}
                            className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg transition-all ${isWaived ? 'bg-amber-500 text-white hover:bg-amber-600' : 'bg-slate-200 text-slate-500 hover:bg-slate-300'} disabled:opacity-50`}>
                            {isWaived ? 'Waived' : 'Waive'}
                          </button>
                          {!isFinalized && (
                            <button onClick={() => handleRemoveFromSeason(p)} className="p-1 text-slate-300 hover:text-red-500 transition-colors" title="Remove from season">
                              <Trash2 size={12} />
                            </button>
                          )}
                        </div>
            ...
            {/* Add Existing Players (2 cols) */}
            <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="font-black text-slate-800 text-sm flex items-center gap-2 mb-3">
                <UserPlus size={16} className="text-blue-600" /> Assign Existing
              </h3>
              <p className="text-[10px] text-slate-400 font-bold mb-3">Players not yet in the {selectedSeason} season.</p>
              ...
              {unassignedPlayers.length === 0 ? (
                ...
                <>
                    ...
                    {unassignedPlayers.map(p => (
                      <button key={p.id} onClick={() => selectedPlayers.includes(p.id) ? setSelectedPlayers(selectedPlayers.filter(id => id !== p.id)) : setSelectedPlayers([...selectedPlayers, p.id])}
                        className={`w-full flex items-center justify-between p-2.5 rounded-lg border transition-all text-left ${
                          selectedPlayers.includes(p.id) ? 'border-blue-500 bg-blue-50' : 'border-slate-100 bg-slate-50 hover:border-slate-200'
                        }`}>
                        <div className="flex items-center gap-2">
                          <span className="w-6 h-6 rounded bg-slate-200 text-slate-500 flex items-center justify-center text-[10px] font-black">{p.jerseyNumber || '?'}</span>
                          <span className="font-bold text-slate-700 text-xs">{p.firstName} {p.lastName}</span>
                        </div>
                  ...
                  {selectedPlayers.length > 0 && (
                    <button onClick={handleImportRoster} disabled={isSaving}
                      className="mt-3 w-full py-2.5 bg-blue-600 text-white font-black rounded-xl shadow-lg hover:bg-blue-700 text-xs disabled:opacity-50">
                      Assign {selectedPlayers.length} Player{selectedPlayers.length !== 1 && 's'} to {selectedSeason}
                    </button>
                  )}
                </>
              )}

              {/* Divider + New Player shortcut */}
              {!isFinalized && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <button onClick={() => setShowPlayerForm(true)}
                    className="w-full py-2.5 border-2 border-dashed border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-300 font-black text-xs rounded-xl transition-all flex items-center justify-center gap-1.5">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-xl font-black text-slate-800 mt-1">{formatMoney(projections.totalBudgeted)}</p>
              <p className="text-[10px] text-slate-400 mt-0.5">{projections.pastSeasons.length} season{projections.pastSeasons.length !== 1 && 's'}</p>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Avg Actual Spend</p>
              <p className="text-xl font-black text-slate-800 mt-1">{formatMoney(projections.totalActualAvg)}</p>
            </div>
            {!isFinalized && projections.suggestedItems.length > 0 && (
              <div className="bg-violet-50 p-4 rounded-2xl border border-violet-200 shadow-sm flex flex-col justify-center">
                <button onClick={handleApplySuggestions} className="w-full py-2.5 bg-violet-600 hover:bg-violet-700 text-white font-black rounded-xl text-xs flex items-center justify-center gap-1.5 transition-all">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="space-y-3">
              {BUDGET_CATEGORIES.filter(c => c.type === 'expense').map(cat => {
                ...
                return (
                  <div key={cat.code} className="bg-slate-50 p-4 rounded-xl">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`text-[10px] font-black ${isOver ? 'text-red-500' : isUnder ? 'text-emerald-500' : 'text-slate-400'}`}>
                          {p.variance > 0 ? '+' : ''}{Math.round(p.variance)}%
                        </span>
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-grow h-2 bg-slate-200 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${(p.avgBudgeted / maxVal) * 100}%` }} />
                        </div>
                        <span className="text-[10px] font-bold text-slate-600 w-16 text-right">{formatMoney(p.avgBudgeted)}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] font-bold text-slate-400 w-14">Actual</span>
                        <div className="flex-grow h-2 bg-slate-200 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full transition-all ${isOver ? 'bg-red-400' : 'bg-emerald-400'}`} style={{ width: `${(p.avgActual / maxVal) * 100}%` }} />
                        </div>
                        <span className={`text-[10px] font-bold w-16 text-right ${isOver ? 'text-red-600' : 'text-slate-600'}`}>{formatMoney(p.avgActual)}</span>
                      </div>
                    </div>
                    {isOver && (
                      <div className="mt-2 flex items-start gap-1.5">
                        <Lightbulb size={11} className="text-amber-500 shrink-0 mt-0.5" />
                        <p className="text-[10px] text-amber-700 font-medium">Consider budgeting ~{formatMoney(p.suggested)} for this category.</p>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="space-y-2">
              {BUDGET_CATEGORIES.filter(c => c.type === 'income').map(cat => {
                const p = projections.categoryProjections[cat.code];
                if (!p || (p.avgBudgeted === 0 && p.avgActual === 0)) return null;
                return (
                  <div key={cat.code} className="flex justify-between items-center p-3 bg-emerald-50 rounded-xl">
                    <div>
                      <span className="text-xs font-bold text-slate-700">{cat.name}</span>
                      <p className="text-[10px] text-slate-400">Avg received: {formatMoney(p.avgActual)}</p>
                    </div>
                    <span className="text-sm font-black text-emerald-600">{formatMoney(p.avgBudgeted)}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <form onSubmit={handleCreateSeason} className="space-y-4">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowNewSeasonModal(false)} className="flex-1 py-2.5 font-bold text-slate-500 hover:bg-slate-50 rounded-xl">Cancel</button>
                <button type="submit" disabled={isSaving} className="flex-1 py-2.5 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 shadow-lg">Create</button>
              </div>
            </form>
          </div>
        </div>
