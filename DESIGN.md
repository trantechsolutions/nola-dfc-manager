...
- **2026-07-21** — Brand identity: **Touchline → Cantera Manager**. Swapped the product name and mark across the app — nav name fallbacks ([DesktopSidebar](src/components/DesktopSidebar.jsx), [MobileHeader](src/components/MobileHeader.jsx), [MobileMenu](src/components/MobileMenu.jsx)), the [login](src/views/general/LoginView.jsx) wordmark + inline mark, [index.html](index.html), [manifest.json](public/manifest.json), and the [service worker](public/service-worker.js) push-title fallback — and regenerated the full favicon/PWA icon set (SVG + 16/32/48 PNG + `.ico` + apple-touch 180 + android-chrome 192/512 full-bleed for maskable) from the new mark: the **cantera gem** (the ball's pentagon cut like quarried stone) nested inside the open **management ring** — the youth academy held within the layer that runs it. A club's _cantera_ is its youth academy (Spanish, "quarry"), and in world football the _manager_ is the one who runs the club — the name is bilingual-native, fits the queued Spanish release, and is unique/ownable. Full name **Cantera Manager** (manifest `name`, `<title>`, login lockup); home-screen `short_name` + apple title stay short as **Cantera**. Palette unchanged — terracotta/clay already read as fired earth + stone; `theme_color` stays slate `#0f172a`. Candidate brand kits live in [brand/](brand/) (`cantera-*` shipped; `touchline-*`/`keeper-*`/`linekeeper-*`/`alternatives-*` kept for reference).
...
- **2026-08-07** — Public landing page + marketing/app host split. Added [LandingView](src/views/general/LandingView.jsx) — a night-match pitch treatment (mow stripes, drawn markings, floodlight washes in [landing.css](src/views/general/landing.css)) over the existing tokens, with copy in a new `landing` i18n namespace in both locales. Two things it taught us: (1) **slash-opacity on token colors renders transparent** — the palette stores finished hex values (`--primary: #0d6efd`), not channel triplets, so `bg-primary/10` and `bg-sidebar/95` compile to invalid CSS and silently disappear. Translucent takes on a token must use `color-mix`. The rest of the app still has unaudited uses of this. (2) The dark hero sits on `--sidebar`, the one token that is dark in both themes, so a night-match slab does not become an "always-dark" one-off (§11). Shipped as its own Vite entry (`landing.html` + [landing-main.jsx](src/landing-main.jsx)) mapped by host in [vercel.json](vercel.json): apex → marketing, `app.*`/`portal.*` → product. The landing bundle carries neither the app nor its vocabulary — `useT` moved to [src/i18n/useT.js](src/i18n/useT.js) so importing the hook no longer drags every namespace in both locales (~105 kB gzip) along with it.
