...
-- Guardians deliberately have no UPDATE policy on players/guardians above:
-- RLS gates whole rows, so any policy letting a parent edit birthdate/shirt
-- size/siblings or a guardian's phone would also expose club_id, team_id,
-- status, and guardian email (which drives the login match in
-- user_guardian_player_ids()) on the same row. Scoped SECURITY DEFINER RPCs
-- instead — see sql/add_guardian_self_service_rpc.sql for the full rationale.
